...
public class Ranlux extends RandomSeedable {
     ...
     */
    public static final int MAX_LUXURY_LEVEL = 4;
    ...
    private void rluxgo(int lux, int ins) {
        int ilx, i, k;

        if (lux < 0) {
            luxlev = lxdflt;
        } else if (lux <= MAX_LUXURY_LEVEL) {
            luxlev = lux;
        } else if (lux < 24 || lux > 2000) {
            luxlev = MAX_LUXURY_LEVEL;
            diag("RANLUX ILLEGAL LUXURY RLUXGO: " + lux);
        } else {
            luxlev = lux;
            for (ilx = 0; ilx <= MAX_LUXURY_LEVEL; ilx++)
